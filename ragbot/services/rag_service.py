...
class RAGService:
    ...
    def __init__(
        ...
        if getattr(settings, "enable_multi_tenant", False):
    ...
    def _initialize_plugin_manager(self):
        ...
        try:
            ...
            if settings.plugins.auto_load:
                logger.info("Auto-loading plugins enabled")
    ...
    async def load_plugin(
        ...
        Returns:
            Result dictionary with success status and plugin ID
        """
        try:
            if not self.plugin_manager:
    ...
    async def get_plugin_status(self, plugin_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if status:
                return {"success": True, "plugin_id": plugin_id, "status": status.value}
            else:
    ...
    async def create_tenant(
        ...
        try:
            ...
            from ragbot.multi_tenant.models import TenantTier, TenantPlan
    ...
    async def get_tenant_usage_trends(
        ...
        """دریافت روند استفاده tenant"""
        try:
            if not self.tenant_analytics:
                return {"error": "Multi-tenant support is disabled"}

            return await self.tenant_analytics.get_usage_trends(tenant_id, days, metric)
        except Exception as e:
    ...
    async def authenticate_tenant_user(
        ...
        try:
            ...
            success, user, session_token = await self.tenant_auth.authenticate_user(
                tenant_id=tenant_id,
                username=username,
                password=password,
            )
            ...
            if success:
                ...
                return {"error": session_token}  # error message
        except Exception as e:
