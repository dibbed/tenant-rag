...
class TestPerformanceOptimizationCycles:
    """تست چرخه بهینه‌سازی عملکرد و پایش سبک"""

    @pytest.mark.asyncio
    async def test_performance_monitor_basic_summary(self):
        monitor = PerformanceMonitor()
    ...
    @pytest.mark.asyncio
    async def test_resource_monitor_summary_and_alerts(self):
    ...
    @pytest.mark.asyncio
    async def test_memory_optimizer_stats_and_health(self):
    ...
    @pytest.mark.asyncio
    async def test_async_processor_parallel_and_shutdown(self):
        processor = AsyncProcessor(max_workers=2)
    ...
    @pytest.mark.asyncio
    async def test_optimization_engine_cycle_runs(self):
